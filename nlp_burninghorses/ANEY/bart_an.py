...
################################################################
# OADA LOSS HELPER FUNCTIONS
################################################################
TARGET_ENTITY_RE = re.compile(r"\[([^\]]+)\]([A-Za-z][A-Za-z0-9_-]*)") 

def parse_target_text(target_text: str) -> list[dict[str, str]]:
    '''
    formats TARGET_ENTITY_RE matches into a list of dicts like:
    text = "Mike Tyson"
    type = "PER"
    '''
    entities = []
...
def format_entities_as_target(entities: list[dict[str, str]]) -> str:
    '''
    Turns a list of dicts like:
    text = "Mike Tyson"
    type = "PER"
    back into [Mike Tyson]PER
    '''
    return " ".join(
...
def make_same_type_permutation_targets(target_text: str) -> list[str]:
    '''
    Only gives the BART script the ability to generate equivalent target strings that the future OADA-XE loss can compare against.
    '''
    ...
    for entity_type in type_order:
        type_entities = grouped[entity_type]

        if len(type_entities) <= 1:
            per_type_options.append([type_entities])
        else:
            per_type_options.append(list(itertools.permutations(type_entities)))
...
################################################################
# BART TRAINING AND EVALUATION CODE
################################################################
...
###########################################################
# COLLATE FUNCTIONS FOR BART TRAINING AND OADA-XE INFERENCE
###########################################################

def make_oada_xe_collate_fn(tokenizer, max_source_length: int):
    def collate_fn(batch: list[dict[str, str]]) -> dict[str, Any]:
        ...
        candidate_targets = [
            make_same_type_permutation_targets(row["target_text"])
            for row in batch
        ]
...
###########################################################
# BATCHED OADA-XE LOSS FUNCTION
# 
# compute loss against canonical target
# compute loss against every equivalent target
# pick the lowest equivalent-target loss
# blend normal XE and OADA-XE using tau
# 
# For the batch:
# average all example losses
###########################################################

def compute_seq2seq_loss_for_target_texts(
    model,
    tokenizer,
...
def oada_xe_loss_batch(
    ...
    for example_index in range(len(canonical_targets)):
        single_input_ids = input_ids[example_index : example_index + 1]
        single_attention_mask = attention_mask[example_index : example_index + 1]

        all_targets = [canonical_targets[example_index]] + [
            target
            for target in candidate_targets[example_index]
            if target != canonical_targets[example_index]
        ]

        target_losses = []

        for target_text in all_targets:
            target_loss = compute_seq2seq_loss_for_target_texts(
                model=model,
                tokenizer=tokenizer,
        ...
        normal_xe = target_losses[0]
        oada_xe = torch.stack(target_losses).min()

        example_loss = (1.0 - tau) * normal_xe + tau * oada_xe
...
def get_oada_tau(
    ...
    progress = min(global_step / warmup_steps, 1.0)
    return tau_start + progress * (tau_end - tau_start)


###########################################################
# EVALUATION AND TRAINING CODE FOR BART
###########################################################
...
def train_one_bart_model(config: BartRunConfig) -> dict[str, Any]:
    ...
    if config.loss_type == "oada_xe":
        print(f"Using OADA tau: {config.oada_tau_start} -> {config.oada_tau_end} (warmup: {config.oada_tau_warmup_steps} steps)")
    ...
    while global_step < config.max_steps:
        ...
        progress_bar.set_postfix_str(
            f"loss={last_loss:.3f} tau={current_tau:.2f} best={best_span_f1:.3f}@{best_step} pat={patience_left}"
        )
