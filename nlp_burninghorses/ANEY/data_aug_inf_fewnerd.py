...
if __package__:
    from .data_aug_train_fewnerd import (
        DEFAULT_DATASET,
    ...
    from nlp_burninghorses.ANEY.data_aug_train_fewnerd import (
        DEFAULT_DATASET,
        DEFAULT_OUTPUT_BASE_DIR,
        DEFAULT_PATTERN_SECTION,
        PROJECT_ROOT,
...
DEFAULT_SPLIT_FILES = {
    "mini_val": "data/interim/fewnerd_supervised_fine_bio_kshot_bert/mini_val.jsonl",
    "validation": "data/interim/fewnerd_supervised_fine_bio_kshot_bert/validation.jsonl",
    "test": "data/interim/fewnerd_supervised_fine_bio_kshot_bert/test.jsonl",
}

DEFAULT_INFERENCE_OUTPUT_DIR = "inference/first_to_last"

def load_fewnerd_id_to_label_from_metadata() -> dict:
    metadata_path = (
        PROJECT_ROOT
        / "data/interim/fewnerd_supervised_fine_bio_kshot_bert/metadata.json"
    )

    with metadata_path.open("r", encoding="utf-8") as f:
        metadata = json.load(f)

    return metadata["id_to_label"]
...
def main() -> None:
    ...
    if dataset_name == "fewnerd":
        id_to_string_map = load_fewnerd_id_to_label_from_metadata()
    else:
        mapping_path = Path(
            os.environ.get(
                "AUG_MAPPING_PATH",
