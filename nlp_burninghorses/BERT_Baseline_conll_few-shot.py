# CoNLL-2003: greedy K-shot train split (Yang & Katiyar 2020, Alg. 1), validation for dev eval.
# Early stopping on dev Span_Strict_F1 every N steps after a warmup (not token accuracy).
# Data: BramVanroy/conll2003 (Parquet; works with current `datasets`). Multiple K values, 3 runs per K.
# From repo root: ./venv/bin/python "nlp_burninghorses/BERT_Baseline_conll_few-shot copy.py"
...
def load_conll2003_datasets() -> DatasetDict:
    """CoNLL-2003 train + validation (dev) from HuggingFace Hub (same source as BERT_Baseline_conll_parquet.py)."""
...
results_filename = "BERT_conll_fewshot_results.csv"
...
def main() -> None:
    ...
    raw_datasets = load_conll2003_datasets()
    raw_datasets = DatasetDict(
        train=raw_datasets["train"].select_columns([text_column_name, label_column_name]),
        validation=raw_datasets["validation"].select_columns([text_column_name, label_column_name]),
    )
        ...
        for k_shot in K_VALUES:
            ...
            for run_index in range(NUM_RUNS_PER_K):
                ...
                model_dir = os.path.join(
                    _repo_root(),
                    "models",
                    f"conll_fewshot_k{k_shot}_run{run_index}",
                )
