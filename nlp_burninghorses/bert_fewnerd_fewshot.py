...
DATA_ROOT = repo_root() / "data" / "interim" / "fewnerd_supervised_fine_bio_kshot_bert"

EXPERIMENT_NAME = "bert_fewnerd_supervised_fine_bio_kshot"

RESULTS_FILENAME = f"{EXPERIMENT_NAME}_results.csv"

MODELS_ROOT = repo_root() / "models" / EXPERIMENT_NAME
RESULTS_ROOT = repo_root() / "results" / EXPERIMENT_NAME
...
def main() -> None:
    ...
    mini_val_ds = dataset_from_jsonl(DATA_ROOT / "mini_val.jsonl")
    ...
    processed_shared_eval = shared_eval_dd.map(
        ...
        batched=True,
        remove_columns=mini_val_ds.column_names,
        desc="Tokenize shared eval splits",
    )

    mini_val_dataloader = make_dataloader(
        processed_shared_eval["mini_val"],
        tokenizer,
        EVAL_BATCH_SIZE,
    )
    ...
    for run_index, split_dir in enumerate(split_dirs):
        ...
        eval_loaders = {
            "mini_val": (mini_val_dataloader, len(mini_val_ds)),
        ...
        for eval_split, (loader, n_eval) in eval_loaders.items():
            ...
            row["dataset"] = "fewnerd"
            row["dataset_config"] = "supervised"
            row["label_level"] = "fine"
            row["tagging_scheme"] = "BIO"
